...
public class JdbcStudentRepository implements StudentsRepository {
    ...
    public void delete(Long id) {
        transactionTemplate.executeWithoutResult(
                status -> {
                    int affectedRows =
                            namedParameterJdbcTemplate.update(
                                    DELETE_STUDENT_QUERY, new MapSqlParameterSource("id", id));
                    if (affectedRows == 0) {
    ...
    private void assignGeneratedIds(
            List<Student> students, List<Map<String, Object>> generatedKeys) {
        ...
        for (int index = 0; index < students.size(); index++) {
            ...
            if (!(generatedId instanceof Number number)) {
                throw new IllegalStateException(
                        "Student ID was not generated for batch row " + index);
            }
    ...
    private void validateBatch(int[] affectedRows, int expectedRows) {
        if (affectedRows.length != expectedRows) {
            throw new JdbcUpdateAffectedIncorrectNumberOfRowsException(
                    INSERT_STUDENT_QUERY, expectedRows, affectedRows.length);
        }

        for (int affectedRow : affectedRows) {
            if (affectedRow != 1 && affectedRow != Statement.SUCCESS_NO_INFO) {
                throw new JdbcUpdateAffectedIncorrectNumberOfRowsException(
                        INSERT_STUDENT_QUERY, 1, affectedRow);
            }
        }
    }
